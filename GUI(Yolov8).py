...
class YOLOv8_GUI:
    ...
    def create_widgets(self):
        # Model Selection
        self.model_label = tk.Label(self.root, text="Select YOLOv8 Model:")
        ...
        self.browse_model_button = tk.Button(self.root, text="Browse", command=self.load_model)
        ...
        self.browse_image_button = tk.Button(self.root, text="Browse", command=self.load_image)
        self.browse_image_button.grid(row=1, column=2, padx=10, pady=10)
        
        # Inference Button
        self.run_button = tk.Button(self.root, text="Run Inference", command=self.run_inference)
        self.run_button.grid(row=2, column=1, padx=10, pady=10)
        
        # Result Display
        self.result_label = tk.Label(self.root, text="Inference Results:")
    ...
    def load_model(self):
        ...
        if model_path:
            ...
            self.model_entry.insert(tk.END, model_path)
    ...
    def display_results(self, results):
        ...
        if isinstance(results, list):
            ...
            for idx, result in enumerate(results):
                self.result_text.insert(tk.END, f"Result {idx + 1}:\n")
